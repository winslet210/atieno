...
with tab2:
    ...
    if staff_names and dept_options: # Add this check
        ...
        if staff_to_update_name:
            ...
            if not dept_list.empty:
                 try:
                     # Ensure the department ID exists in dept_list before finding the index
                     if current_staff_info["department_id"] in dept_list["id"].values:
                        current_dept_index = dept_list[dept_list["id"] == current_staff_info["department_id"]].index[0]
                     else:
                         st.warning(f"Department ID {current_staff_info['department_id']} for staff {staff_to_update_name} not found in departments list. Defaulting to first department.")
                         current_dept_index = 0
                 except IndexError:
                     current_dept_index = 0 # Default to first if department ID not found
